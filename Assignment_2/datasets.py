...
class dataset(Dataset):
    ...
    def __getitem__(self, idx): # 1.25 Mark
        ...
        statement = self.dataframe.iloc[idx, 3].lower()
        justification = self.dataframe.iloc[idx,15].lower()
        statement = word_tokenize(statement)
        justification = word_tokenize(justification)

        label = torch.tensor(self.labels[self.dataframe.iloc[idx, 2]]).float()
        ...
        vectorized_statement = torch.tensor(vectorized_statement).float()
        vectorized_justification = torch.tensor(vectorized_justification).float()

        credit_history = torch.tensor([self.dataframe.iloc[idx, 12], 
        self.dataframe.iloc[idx, 11], self.dataframe.iloc[idx, 9], 
        self.dataframe.iloc[idx, 10], self.dataframe.iloc[idx, 13]]).float()
