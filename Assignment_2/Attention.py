...
class MultiHeadAttention(nn.Module):
    ...
    def get_layers(self, hid_dim): # 0.5 Marks
        conv_query = con_key = conv_value = nn.Conv1d(hid_dim, hid_dim, 3, 1, 1)
        conv_output = nn.Conv1d(hid_dim, hid_dim, 1, 1, 0)
        return conv_query, con_key, conv_value, conv_output
    """
...
class PositionFeedforward(nn.Module):
    ...
    def forward(self, x): # 1 Mark
        ...
        x_backup= torch.add(x_backup, out_2)
        x = x_backup
        # Your code goes here. 
    ...
    def get_layers(self, hid_dim, feedForward_dim): # 0.5 Mark
        ...
        conv2 = nn.Conv1d(feedForward_dim, hid_dim, 1, 1, 0)
        return [conv1, conv2]